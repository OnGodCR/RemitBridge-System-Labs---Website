...
**Reading** — `/blog` (30 planned, 7 written) · `/papers` (one, drafting)
...
1. **Data source.** World Bank RPW is the real option and is now loaded:
   firm-level fees, FX margins and speed across 48 sending and 105 receiving
   countries, in `public/data/rpw`, updated by rerunning `scripts/rpw.mjs` on
   each quarter's spreadsheet (no access form; it is a direct download from the
   Data Catalog). Prices are a quarter behind. The decision left is whether a
   live overlay is wanted on top.
...
- Corridor data is in. `public/data/rpw/` holds the World Bank survey as one
  file per corridor, generated by `scripts/rpw.mjs` from the spreadsheet in
  `data/raw/` (ignored, 50 MB, from the World Bank Data Catalog, dataset
  0037898). TrueCost fetches a corridor's file when a reader prices that
  pair; where a currency pair means several corridors (EUR to MAD is six),
  the reader picks. To update: download the new quarter, run the script,
...
- Blog — 30 planned, 17 written. The page counts entries in `postBodies.jsx`
