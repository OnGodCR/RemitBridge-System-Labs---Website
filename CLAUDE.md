...
- [ ] **One figure per section is the standing rule for blog posts**, set
      2026-08-21. Posts 2 through 13 all meet it. Post 2's corridor map and post
...
- [x] Corridor data, all of it. 348 corridors priced in Q3 2025, 368 with
      history back to 2016, from the World Bank's own spreadsheet via
      `scripts/rpw.mjs` into one static file per corridor under
      `public/data/rpw`. Nothing in the bundle; TrueCost fetches the one
      corridor a reader prices. The corridor average reproduces the survey
      site to the decimal. Done 2026-09-21. New quarter: drop the spreadsheet
      in `data/raw/`, run the script, commit the output.
...
The World Bank corridor survey lives in `public/data/rpw`, generated by
`scripts/rpw.mjs` from a spreadsheet that stays out of git. Its terms require
the attribution line wherever the data shows; it ships inside every file as
`vintage.attribution`, and the quarter renders beside every figure. Never
describe the World Bank as a partner or as endorsing anything here.
